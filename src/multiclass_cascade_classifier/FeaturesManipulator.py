...
def features_truncate(X_res):
    ...
    """
    X_res_txt = X_res[X_res.columns.to_list()[:-nb_bin_features]]
    X_res_txt = [row[:-nb_bin_features] for row in X_res.values.tolist()]
    X_res_txt = pd.DataFrame(X_res_txt, columns=X_res.columns.to_list()[:-nb_bin_features], index=X_res.index)
    ...
    X_res_bin = [row[-nb_bin_features:] for row in X_res.values.tolist()]
    X_res_bin = pd.DataFrame(X_res_bin, columns=X_res.columns.to_list()[-nb_bin_features:], index=X_res.index)
...
def features_intersection(X, features):
    ...
    features_text = features[:-nb_bin_features]
    
    X_columns_text = X.columns.to_list()[:-nb_bin_features]
    X_columns_bin = X.columns.to_list()[-nb_bin_features:]
    
    X_text = [row[:-nb_bin_features] for row in X.values.tolist()]
    X_text = pd.DataFrame(X_text, columns=X_columns_text, index=X.index)
    X_bin = [row[-nb_bin_features:] for row in X.values.tolist()]
